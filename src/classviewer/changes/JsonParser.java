...
public class JsonParser {
	...
	public ArrayList<Object> parse(Reader reader) throws IOException {
	 ...
	 */
	private Object parseObject(StringBuffer buf, int[] idx) {
		if (buf.length() == 0)
			return null;
		if (buf.charAt(idx[0]) == '[') {
			...
			while (v >= 0 && v + 6 <= str.length()) {
				char c = (char) Integer.parseInt( str.substring(v+2, v+6), 16 );
				str = str.substring(0, v) + c + str.substring(v+6);
				v = str.indexOf("\\u");
			}
			return str;
		} else if ("null".equals(buf.substring(idx[0], idx[0] + 4))) {
			idx[0] += 4;
			return null;
		} else if ("true".equals(buf.substring(idx[0], idx[0] + 4))) {
			idx[0] += 4;
			return Boolean.TRUE;
		} else if ("false".equals(buf.substring(idx[0], idx[0] + 5))) {
			idx[0] += 5;
	...
	private void skipSpace(StringBuffer b, int[] idx) {
		while (idx[0] < b.length() && Character.isWhitespace(b.charAt(idx[0])))
			idx[0]++;
	}

	private ArrayList<Object> parseList(StringBuffer buf, int[] idx) {
		idx[0]++;
	...
	private HashMap<String, Object> parseMap(StringBuffer buf, int[] idx) {
		idx[0]++;
